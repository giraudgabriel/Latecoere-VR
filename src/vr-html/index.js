...
document.onreadystatechange = () => {
  if (document.readyState === "complete") {
    ...
    setaEsquerdaMontagem.addEventListener("click", () =>
      virarPraEsquerda("-montagem")
    );

    //virar pra direita montagem
    setaDireitaMontagem.addEventListener("click", () =>
    ...
    scene.addEventListener("enter-vr", () => {
      ...
      el.object3D.position.set(0, 1, 3);
    });
  }
};
...
function count() {
  passo = passo >= pecas.length ? pecas.length : passo + 1;
}

function countSub() {
  passo = passo < 0 ? 0 : passo - 1;
}
